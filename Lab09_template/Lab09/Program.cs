...
namespace Lab09
{
    class Program
    {
        ...
        /*xml dosyaları için serialize - deserialize metodları
        static void XMLSerialize(University data)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(University));
            FileStream fs = new FileStream("./uni.xml", FileMode.Create);
        ...
        {
            FileStream fs = new FileStream("./uni.xml", FileMode.Open);
            XmlSerializer xs = new XmlSerializer(typeof(University));
            University deu = (University)xs.Deserialize(fs);
            fs.Close();
            return deu;
        }
        */
